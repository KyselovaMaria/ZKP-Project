...
Browser                                    Server
───────────────────────────────────        ──────────────────────
User enters date of birth locally          →  receives: { proof, publicSignals: ["18"] }
JS computes age = today - birthdate        runs: snarkjs.groth16.verify(vk, ...)
snarkjs.groth16.fullProve(                 ←  returns: { verified: true/false }
  { age, threshold: 18 },         
  age_check.wasm,                  
  circuit_final.zkey      
)
