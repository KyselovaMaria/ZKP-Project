  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        ctx.fillStyle=`rgba(99,202,183,${p.a})`; ctx.fill();
      });
      for (let i=0;i<pts.length;i++) for (let j=i+1;j<pts.length;j++) {
        const dx=pts[i].x-pts[j].x, dy=pts[i].y-pts[j].y, d=Math.sqrt(dx*dx+dy*dy);
        if(d<110){ ctx.beginPath(); ctx.moveTo(pts[i].x,pts[i].y); ctx.lineTo(pts[j].x,pts[j].y);
          ctx.strokeStyle=`rgba(99,202,183,${.07*(1-d/110)})`; ctx.lineWidth=.5; ctx.stroke(); }
      }
...
export default function App() {
  ...
  const [proofData, setProofData] = useState(null); // { proof, publicSignals }
  const [result, setResult]     = useState(null);   // { verified, ... }
  ...
  const run = async () => {
    ...
    await delay(400);
    addLog("$ Server running: e(A,B) = e(α,β)·e(Σ pubInputs·IC, γ)·e(C,δ)");
    await delay(300);
        ...
        <div className="grid">
          ...
          <div className="panel">
            ...
            {proofData && (
              <div className="proof-box">
                <div className="sec-title">
                  Proof π <span className="tag-sm">256 bytes</span>
                </div>
                <ProofField label="π_A (G1)" value={proofData.proof.pi_a?.[0]} delay={0} />
                <ProofField label="π_B (G2)" value={proofData.proof.pi_b?.[0]?.[0]} delay={100} />
                <ProofField label="π_C (G1)" value={proofData.proof.pi_c?.[0]} delay={200} />
            ...
            <div className="info-box">
              ...
              <div className="info-row">
                ...
                <span>Server verifies the pairing equation in &lt;10ms</span>
              </div>
            </div>
          </div>
          ...
          <div className="panel">
            ...
            <div className="zk-props">
              {[
                ["∅", "Zero-Knowledge", "Server learns only age≥18, nothing else"],
                ["✓", "Completeness", "Honest prover always succeeds"],
                ["🛡", "Soundness", "Breaking requires solving discrete log on BN128"],
                ["⚡", "Succinctness", "Proof is 256 bytes regardless of circuit size"],
              ].map(([icon, title, desc]) => (
            ...
            {vkInfo && (
              <div className="vk-box">
                <div className="sec-title">Verification Key Info</div>
            ...
            <button className="btn-math" onClick={() => setShowMath(v=>!v)}>
              {showMath ? "▾" : "▸"} Groth16 Verification Equation
            </button>
            ...
            {showMath && (
              <div className="math">
                ...
                <div className="math-eq">
                  e(A, B) =<br/>
                  &nbsp;&nbsp;e(α, β)<br/>
                  &nbsp;&nbsp;· e(∑ w<sub>i</sub>·IC<sub>i</sub>, γ)<br/>
                  &nbsp;&nbsp;· e(C, δ)
                </div>
